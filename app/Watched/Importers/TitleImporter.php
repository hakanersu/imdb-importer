...
class TitleImporter extends Importer implements  ImporterInterface
{
    ...
    public function start($path = null): ImporterInterface
    {
        $now = now();

        $this->output->writeln('<info>Importing titles.</info>');
        if ($path) {
            $this->tsvPath = $path."/tmp/imdb/".$this->name;
        }
    ...
    public function index(): ImporterInterface
    {
        ...
        $this->output->writeln('<info>Title cleanup and indexing.</info>');
